...
Vagrant.configure("2") do |config|
  ...
  if OS.windows? || OS.linux?
    config.vm.synced_folder "./", "/var/git/kafoso/type-formatter", create: true, type: "nfs"
  else
    config.vm.synced_folder "./", "/var/git/kafoso/type-formatter", create: true, owner: "vagrant", group: "www-data", mount_options: ["dmode=775,fmode=664"]
  end
